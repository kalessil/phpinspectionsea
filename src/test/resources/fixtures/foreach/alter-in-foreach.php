...
function AIFTestCases() {
    ...
    foreach ($source as <error descr="Provokes PHP Fatal error (key element cannot be a reference)">&</error> $id => $element) {
        <weak_warning descr="Can be refactored as '$element = ...' if $element is defined as reference (ensure that array supplied). Suppress if causes memory mismatches.">$source[$id]</weak_warning>
            = $element + 1;
    }
    unset(
