...
public abstract class GenericPhpElementVisitor extends PhpElementVisitor {
    ...
    final public void visitPhpElement(@NotNull PhpPsiElement element) {
        if (element instanceof PhpDocTag) {
            ...
            if (OpenapiTypesUtil.isThrowExpression(element)) {
                // PS 2020.3, 2021.1 has changed the throw structure, hence we are mimic the latest visitor.
    ...
    protected boolean isTestContext(@NotNull PsiElement expression) {
        ...
        if (filePath.endsWith("Test.php") || filePath.endsWith("Spec.php") || filePath.endsWith(".phpt") || filePath.contains("/Fixtures/")) {
            ...
            final PhpClass containingClass = expression instanceof PhpClass
                    ? (PhpClass) expression
                    : PsiTreeUtil.getParentOfType(expression, PhpClass.class, false, PsiFile.class);
