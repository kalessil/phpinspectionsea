...
final public class CanBeReplacedWithArraySearchStrategy extends AbstractStrategy {
    static public boolean apply(@NotNull ForeachStatement foreach, @NotNull PsiElement expression, @NotNull Project project) {
        ...
        final PsiElement index = foreach.getKey();
        final PsiElement value = foreach.getValue();
        if (index != null && value != null && expression instanceof If) {
                    ...
                    if (last instanceof PhpBreak) {
                        ...
                        if (OpenapiTypesUtil.isStatementImpl(first)) {
                            ...
                            if (OpenapiTypesUtil.isAssignment(candidate)) {
                                return isTargetValue(index, ((AssignmentExpression) candidate).getValue()) &&
    ...
    static private boolean isTargetValue(@NotNull PsiElement index, @Nullable PsiElement value) {
        return value != null && OpenapiEquivalenceUtil.areEqual(value, index);
    }
