                        ...
                        if (!candidate.isEmpty()) {
                            ...
                            if (candidate.matches("^\\\\[dDwWsS][*+]$")) {
                                holder.registerProblem(
                                        target,
                                        String.format(MessagesPresentationUtil.prefixWithEa(patternCompound), candidate, matcher.group(3)),
                    ...
                    if (!fragments.isEmpty()) {
                        if (fragments.size() >= 2) {
                            if (fragments.contains("\\d") && fragments.contains("\\w")) {
                                holder.registerProblem(
                                        target,
                                        String.format(MessagesPresentationUtil.prefixWithEa(patternExclusive), "\\d", "\\w", fragment),
                                        ProblemHighlightType.GENERIC_ERROR
                                );
                                result = true;
