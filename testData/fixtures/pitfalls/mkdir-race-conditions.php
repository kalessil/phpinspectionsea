...
function cases_holder() {
    ...
    <error descr="[EA] Following construct should be used: 'if (!mkdir('...', recursive: true) && !is_dir(...)) { ... }'.">mkdir('...', recursive: true);</error>
    ...
    if (is_dir('...') || <error descr="[EA] Some check are missing: 'mkdir('...', 0644) || is_dir(...)'.">mkdir('...', 0644)</error>) {}
    if (!is_dir('...') && <error descr="[EA] Some check are missing: '!mkdir('...', recursive: true) && !is_dir(...)'.">!mkdir('...', recursive: true)</error>) {}
    if (is_dir('...') || <error descr="[EA] Some check are missing: 'mkdir('...', recursive: true) || is_dir(...)'.">mkdir('...', recursive: true)</error>) {}
...
function quickfix_with_variable() {
    ...
    if (<error descr="[EA] Some check are missing: 'mkdir(trim('...'), recursive: true) || is_dir(...)'.">(mkdir(trim('...'), recursive: true))</error>) {}
}